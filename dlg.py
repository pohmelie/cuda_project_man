...
def dialog_config(op):
    ...
    text = '\n'.join([]
        ...
        +[c1.join(['type=button', 'pos=300,270,400,0', 'cap=&OK', 'props=1'])]
        +[c1.join(['type=button', 'pos=406,270,502,0', 'cap=Cancel'])]
    )

    res = dlg_custom('Project Manager options', 508, 300, text)
    if res is None:
        return
